...
export default function AssetDialog() {
  ...
  const assetsWithoutExcludes = assets.data.filter((asset) => !excludedAssets.includes(asset.id));

  const visibleAssets = showOnlySelectedAssets ? selectedAssetsSnapshot : assetsWithoutExcludes;
  ...
  return (
    <DialogContent padding="0" height="48rem">
      <div className="flex h-full flex-col justify-between">
                  ...
                  <TableHeader className="w-[60px]">
                    {!isSingleSelect ? (
                      <div className="grid place-items-center">
                        <input
                          type="checkbox"
                          checked={selectedAssets.length === visibleAssets.length}
                          onChange={() => {
                            if (selectedAssets.length === visibleAssets.length) {
                              return setSelectedAssets([]);
                            }

                            setSelectedAssets(visibleAssets);
                          }}
                        />
                      </div>
                    ) : null}
        ...
        <div className="flex justify-end px-24 py-16">
          {!isSingleSelect ? (
